...
test_that("trying to upload non-exitent file errors", {
  ...
  expect_error(volume$upload(key$fid, "/path/to/file"),
...
test_that("download non-exitent file throws error", {
  ...
  expect_error(volume$download(key$fid), "Client error: (404) Not Found",
               fixed = TRUE)
})
